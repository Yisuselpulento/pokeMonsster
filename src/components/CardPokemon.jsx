...
const CardPokemon = ( {pokemon} ) => {
  ...
  return (
    <Link 
    to={`/${name}`}
    className={` px-2  pb-4 flex flex-col items-center rounded w-[155px] h-[230px] justify-between border border-neutral-700 bg-gradient-to-b from-neutral-800 to-transparent  ${colorByType[colorBg]} bg-opacity-10 hover:scale-105 hover:bg-opacity-20`}>
    ...
    {  <div className="flex gap-2">
        {types.map((tipo,i) => (
          <p 
          className={`${colorByType[tipo.type.name]} rounded-full py-1 px-2`}
